using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.IO.Ports;
using System.Text;
using System.Threading;

namespace KillerApps.AtariLynx.Tooling.Flashcart
{
    public class FlashcartProxy
    {
        private const int BUFFER_SIZE = 256;
        private const int WRITE_CHUNKSIZE = 64;
        private const int RECEIVED_BYTES_THRESHOLD = 1;
        private const int READ_TIMEOUT = 5000;
        
        public const string FLASHCART_SYSTEMINFO = "s";
        
        public event ProgressChangedEventHandler ProgressChanged;
        
        private StringBuilder logBuilder = new StringBuilder();
        private WriteStatus status = new WriteStatus();

        public bool Verbose { get; set; }

        public string SendMessageAndReceiveText(string portName, int baudRate = 115200)
        {
            using (SerialPort port = new SerialPort(portName, baudRate, Parity.None, 8, StopBits.One))
            {
                port.ReceivedBytesThreshold = RECEIVED_BYTES_THRESHOLD;
                port.DataReceived += OnDataReceived;
                if (!port.TryOpen()) return String.Empty;
                port.WriteByte((byte)'w');

                Thread.Sleep(500);
                string text = logBuilder.ToString();
                return text;
            }
        }
        
        EventWaitHandle waitEraseCompleted;

        public string WriteRomFile(string portName, int baudRate, byte[] content)
        {
            using (SerialPort port = new SerialPort(portName, baudRate, Parity.None, 8, StopBits.One))
            {
                waitEraseCompleted = new EventWaitHandle(false, EventResetMode.ManualReset);
                status.TotalBytes = content.Length;
                port.DataReceived += OnDataReceived;
                
                if (!port.TryOpen()) return String.Empty;
                port.WriteByte((byte)'w');
                
                if (!waitEraseCompleted.WaitOne(5000))
                {
                    Console.WriteLine("Timeout during erase");
                    //return String.Empty;
                }
                if (Verbose) Console.Write(status.Log);

                int bytesSent = 0;
                while (bytesSent < content.Length)
                {
                    // Send single chunk
                    int chunkSize = Math.Min(content.Length - bytesSent, WRITE_CHUNKSIZE);
                    port.Write(content, bytesSent, chunkSize);
                    bytesSent += chunkSize;

                    // Report progress
                    int percentage = (bytesSent * 100) / content.Length;
                    status.BytesWritten = bytesSent;

                    ProgressChanged?.Invoke(this, new ProgressChangedEventArgs(percentage, status));
                }

                string text = logBuilder.ToString();
                return text;
            }
        }

        void OnDataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            SerialPort port = (SerialPort)sender;
            byte[] buffer = new byte[BUFFER_SIZE];
            int bytesRead = port.Read(buffer, 0, BUFFER_SIZE);

            logBuilder.Append(ASCIIEncoding.ASCII.GetString(buffer, 0, bytesRead));
            status.Log = logBuilder.ToString().Split("\r\n")[0];
            if (logBuilder.ToString().StartsWith("erasing memory............\r\nplease start upload data\r\n"))
                { waitEraseCompleted.Set(); }
            ProgressChanged?.Invoke(this, new ProgressChangedEventArgs(status.BytesWritten, status));
        }
    }

    public class WriteStatus
    {
        public string Log { get; set; }
        public int BytesWritten { get; set; }
        public int TotalBytes { get; set; }
    }
}

/*
FLASH
content: bin-file
[w] write       [v] verify
93C86 
[u] write       [y] verify      [m] modify byte [r] read        [e] erase
BAUDRATE
[9] 9600        [5] 57600       [X] 115200
MODE
[g] 128k        [X] 256k/bll    [i] 512k        [k] 512k-bll
[l] lnx         [X] bin/lyx     [o] *.o
LANGUAGE
[X] english     [4] deutsch     [6] francais    [7] espanol     [8] nederlands
SYSTEM
[s] systeminfo  [c] credits     [x] reset all
= OK ===========================================================================

// "s", "System Info", "= OK ==========================================================================="
// "c", "Credits",""
// "9", "9600 Baud Set", "= OK ==========================================================================="
// "2", "19200 Baud Set", "= OK ==========================================================================="
// "3", "38400 Baud Set", "= OK ==========================================================================="
// "5", "57600 Baud Set", "= OK ==========================================================================="
// "1", "115200 Baud Set", "= OK ==========================================================================="
// "g", "128K ROM Size Set","");
// "h", "256K/BLL ROM Size Set","");
// "i", "512K ROM Size Set","");
// "k", "512K-BLL ROM Size Set","");
// "b", ".BIN / .LYX ROM Type Set","");
// "l", ".LNX ROM Type Set","");
// "x", "Resetting","FLASH");

*/